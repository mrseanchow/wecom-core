...
type AddServicerRequest struct {
	...
	UserIDList       []string `json:"userid_list,omitempty"`        // 接待人员userid列表,可填充个�?0~100
	DepartmentIDList []uint64 `json:"department_id_list,omitempty"` // 接待人员部门id列表,可填充个�?0~20
}
...
type ServicerOperationResult struct {
	...
	ErrCode      int    `json:"errcode"`                 // 该userid的操作结�?
...
type DeleteServicerRequest struct {
	OpenKfID         string   `json:"open_kfid"`                    // 客服账号ID
	UserIDList       []string `json:"userid_list,omitempty"`        // 接待人员userid列表,可填充个�?0~100
	DepartmentIDList []uint64 `json:"department_id_list,omitempty"` // 接待人员部门id列表,可填充个�?0~100
}
...
type Servicer struct {
	UserID       string `json:"userid,omitempty"`        // 接待人员的userid
	DepartmentID uint64 `json:"department_id,omitempty"` // 接待人员部门的id
	Status       uint   `json:"status,omitempty"`        // 接待人员的接待状�?0:接待�?1:停止接待
...
type ListServicerResponse struct {
	common.Response
	ServicerList []Servicer `json:"servicer_list"` // 客服账号的接待人员列�?
}

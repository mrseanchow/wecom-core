...
type GetAgentResponse struct {
	...
	Close                   int             `json:"close"`                     // 企业应用是否被停用�?：未被停用；1：被停用
	RedirectDomain          string          `json:"redirect_domain"`           // 企业应用可信域名
	ReportLocationFlag      int             `json:"report_location_flag"`      // 企业应用是否打开地理位置上报 0：不上报�?：进入会话上�?
	IsReportEnter           int             `json:"isreportenter"`             // 是否上报用户进入应用事件�?：不接收�?：接�?
...
type SetAgentRequest struct {
	...
	ReportLocationFlag *int   `json:"report_location_flag,omitempty"` // 企业应用是否打开地理位置上报 0：不上报�?：进入会话上�?
	LogoMediaID        string `json:"logo_mediaid,omitempty"`         // 企业应用头像的mediaid
	Name               string `json:"name,omitempty"`                 // 企业应用名称，长度不超过32个utf8字符
	Description        string `json:"description,omitempty"`          // 企业应用详情，长度为4�?20个utf8字符
	RedirectDomain     string `json:"redirect_domain,omitempty"`      // 企业应用可信域名
	IsReportEnter      *int   `json:"isreportenter,omitempty"`        // 是否上报用户进入应用事件�?：不接收�?：接�?
